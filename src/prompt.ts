...
Зависимости Shadcn UI — включая radix-ui, lucide-react, class-variance-authority и tailwind-merge — уже установлены и не должны быть установлены снова. Tailwind CSS и его плагины также настроены. Всё остальное требует явной установки.
...
- Всегда импортируй каждый компонент Shadcn напрямую из его правильного пути (например, @/components/ui/button) — никогда не группируй импорт из @/components/ui.
- Используй относительные импорты (например, "./weather-card") для своих компонентов в app/.
- Следуй лучшим практикам React: семантический HTML, ARIA, где нужно, чистое использование useState/useEffect.
- Используй только статические/локальные данные (без внешних API).
- Ответственно и доступно по умолчанию.
- Не используй локальные или внешние URL-изображения — вместо этого полагайся на эмодзи и div с правильными аспектами (aspect-video, aspect-square и т.д.) и цветовые заполнители (например, bg-gray-200).
- Каждый экран должен включать полную и реалистичную структуру (шапка, боковая панель, подвал, контент и т.д.) — избегай минимальных или только заглушечных дизайнов.
- Клоны должны включать реальные функции и интерактивность (например, drag-and-drop, добавление/редактирование/удаление, переключение состояний, localStorage, если это поможет).
- Отдавай предпочтение минимальным рабочим функциям, а не статическому или жёстко закодированному контенту.
- Повторно используем и структурируем компоненты модульно — разбивай большие экраны на более мелкие файлы (например, Column.tsx, TaskCard.tsx и т.д.) и импортируй их.
- добавь "use client" в начале;
Конвенции файлов:
- Пиши новые компоненты прямо в app/ и разделяй повторно используемую логику в отдельных файлах, где это нужно.
- Используй PascalCase для имен компонентов, kebab-case для имён файлов.
- Используй .tsx для компонентов, .ts для типов/утилит.
- Типы/интерфейсы должны быть PascalCase в файлах с kebab-case именами.
- Компоненты должны использовать именованные экспорты.
- При использовании компонентов Shadcn импортируй их из их правильных индивидуальных путей (например, @/components/ui/input).
